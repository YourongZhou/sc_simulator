...
def CME_corrected(X: np.ndarray[tuple[int, int], np.dtype[np.float32]],
    ...
    cme = CME_cuda(X, normalize, feature_indices)
    null_cme_ary = rand_permute_CME(X, iterations, normalize, feature_indices)
    pval_mtx = CME_pva_cuda(cme, null_cme_ary)
...
def rand_permute_CME(X: np.ndarray[tuple[int, int], np.dtype[np.float32]],
    ...
    for i in range(iterations):
        ...
        X_shuffled = shuffle_and_normalize_cuda(X)

        cme_shuffled = CME_cuda(X_shuffled, normalize=normalize, feature_indices=feature_indices)
